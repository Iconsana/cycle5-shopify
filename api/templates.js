// Create a templates.js file in your api folder// Vercel serverless function for templates API
// This uses Vercel's native API handlers instead of Express

// Hardcoded templates with their SVG content
const templates = [
  {
    id: 'battery-style',
    name: 'Battery Style',
...
export default function handler(req, res) {
  ...
  // Return all templates without content (to keep response size small)
  const templatesWithoutContent = templates.map(({ id, name, file }) => ({
    id,
    name, 
    file
  }));
...
// Get all templates
router.get('/', (req, res) => {
  // Return templates without the full SVG content to keep response size small
  const templatesWithoutContent = templates.map(({ id, name, file }) => ({
    id,
    name,
    file
  ...
  res.json({ success: true, data: templatesWithoutContent });
});
